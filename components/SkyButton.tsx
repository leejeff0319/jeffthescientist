...
function SkyButton() {
  ...
  const setSun = (travelFraction: number) => {
  ...
  const setMoon = (travelFraction: number) => {
    const fullScreenWidthDuration = 4;
    const duration = fullScreenWidthDuration * travelFraction;
  ...
  const handleButtonClick = () => {
    if (isActivated) {
      ...
      setSun(travelFraction);
      setMoon(travelFraction);
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className={`growButton h-20 w-10 fixed top-1/4 right-0 z-50 text-white p-1 rounded-md text-xs flex items-center justify-center ${buttonColor === 'blue-500' ? 'bg-blue-500' : 'bg-black'}`}
      >
